...
calc_dist_203_CRO(){
    if [ ! -d dist_203_CRO ] ; then mkdir dist_203_CRO ; fi 
    cd dist_203_CRO 
    
    if [ ! -f $MOLEC.dist_203_CRO.xvg ] ; then 
        gmx distance -s ../Production/$MOLEC.production.tpr -f ../Production/$MOLEC.production.nopbc.xtc  -select 'com of residue 202 plus (resname CROa and name OH)' -oall $MOLEC.dist_203_CRO.xvg >> $logFile 2>> $errFile 
    fi 
    check $MOLEC.dist_203_CRO.xvg
...
calc_dist_CNF_CRO(){
    ...
    if [ ! -f $MOLEC.dist_CNF_CRO.xvg ] ; then 
        gmx distance -s ../Production/$MOLEC.production.tpr -f ../Production/$MOLEC.production.nopbc.xtc  -select 'com of resname CNF plus (resname CROa and name OH)' -oall $MOLEC.dist_CNF_CRO.xvg >> $logFile 2>> $errFile 
    fi 
