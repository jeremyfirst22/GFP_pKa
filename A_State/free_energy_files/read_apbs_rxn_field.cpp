    ...
    if (file.is_open())
    {
        while (file.good())
        {
            ...
            if (not pqrline.empty())
            {
                ...
                if (std::strncmp(recordtype.c_str(), "ATOM",6) != 0 ) { 
                    continue ; 
                }
                linestream >> throwAway >> atom >> resname >> throwAway >> coords[0] >> coords[1] >> coords[2];
...
double parse_selection( const std::vector<float> &rxnpot, const std::vector<std::vector<float> > &coord, const std::vector<int> &selection ) 
{
    ...
    if (n>0) 
    {
        ...
        double dV = rxnpot[n0] - rxnpot[n1]; // y1-y0/dx = slope
