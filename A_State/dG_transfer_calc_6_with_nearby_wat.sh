...
if [ ! -d 6_NEARBY_WAT_ENERGIES ] ; then mkdir 6_NEARBY_WAT_ENERGIES ; fi 
cd 6_NEARBY_WAT_ENERGIES
...
for frame in $(seq 0 $timeStep $tot) ; do 
    ...
    if ! grep -sq Global time_${frame}_Protein-CRO.out ; then     
        ...
        sed "s/file2.pqr/time_${frame}_Protein+CRO.pqr/" ../../free_energy_files/6template.in > temp.in
