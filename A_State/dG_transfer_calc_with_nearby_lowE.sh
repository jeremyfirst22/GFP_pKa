...
if [ ! -d LOWE_NEARBY_WAT_ENERGIES ] ; then mkdir LOWE_NEARBY_WAT_ENERGIES ; fi 
cd LOWE_NEARBY_WAT_ENERGIES
...
for frame in $(seq 0 $timeStep $tot) ; do 
    ...
    if ! grep -sq Global time_${frame}_Protein-CRO.out ; then     
        ...
        echo $index | gmx trjconv -f $XTC -s $TPR -o time_${frame}.pdb -dump $frame -tu ps >> $logFile 2>> $errFile -n index.ndx 
        ...
        sed "s/file2.pqr/time_${frame}_Protein+CRO.pqr/" ../../free_energy_files/template.in > temp.in
