...
if [ ! -d wat_near_CRO ] ; then mkdir wat_near_CRO ; fi 

for molec in $molecList ; do 
        printf "\n\t$molec\n" 
        if [ -f wat_near_CRO/${molec}.wat_near_CRO.xvg ] ; then printf "\nSkipped\n" ; continue ; fi 
        if [ ! -f ${molec}/Production/${molec}.production.nopbc.xtc ] ; then printf "\nTrajectory not found\n" ; continue ; fi 

        cd $molec/Production
        
        gmx select -f $molec.production.nopbc.xtc -s $molec.production.tpr -os ../../wat_near_CRO/${molec}.wat_near_CRO.xvg -olt ../../wat_near_CRO/${molec}.lifetime.xvg -select 'group Water and same residue as within 0.5 of resname CROn'


        check ../../wat_near_CRO/${molec}.wat_near_CRO.xvg 
        cd ../../
