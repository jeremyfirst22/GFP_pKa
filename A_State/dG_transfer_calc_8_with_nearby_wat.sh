...
if [ ! -d 8_NEARBY_WAT_ENERGIES ] ; then mkdir 8_NEARBY_WAT_ENERGIES ; fi 
cd 8_NEARBY_WAT_ENERGIES
...
for frame in $(seq 0 $timeStep $tot) ; do 
    ...
    if ! grep -sq Global time_${frame}_Protein-CRO.out ; then     
        ...
        sed "s/file2.pqr/time_${frame}_Protein+CRO.pqr/" ../../free_energy_files/8template.in > temp.in
