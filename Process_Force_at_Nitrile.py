...
force_files = glob.glob('[AB]_State/*/%s/*.solvent_rxn_field.projected.xvg'%force_dir) 

molList= []
for item in force_files : 
    state, molec,throwAway,field = item.split('/') 
    if not os.path.isfile("%s/%s/%s/%s.solvent_rxn_field.projected.xvg"%(state,molec,force_dir,molec)) : 
...
for molec in range(numMols ) : 
    if not molList[molec][1] == molList[molec+numMols][1] : 
        ...
        exit() 
    try : 
        fieldA = np.genfromtxt("%s/%s/%s/%s.solvent_rxn_field.projected.xvg"%(molList[molec][0],molList[molec][1],force_dir,molList[molec][1]),comments='#' ) 
        fieldB = np.genfromtxt("%s/%s/%s/%s.solvent_rxn_field.projected.xvg"%(molList[molec+numMols][0],molList[molec+numMols][1],force_dir,molList[molec+numMols][1]),comments='#' ) 
    except : 
