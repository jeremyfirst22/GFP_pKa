...
fileList = glob.glob('A_State/*/LOWE_NEARBY_WAT_ENERGIES/dG_xter_CRO.dat') 
mutList = [] 
...
for index,mut in enumerate(mutList): 
    mut=mutList[index]
    outName = "LowE_Nearby_Wat_%s_ddG.pdf"%mut
    ...
    #print bashCommand
    dataFileA="../A_State/%s/LOWE_NEARBY_WAT_ENERGIES/dG_xter_CRO.dat"%mut
    dataFileB="../B_State/%s/LOWE_NEARBY_WAT_ENERGIES/dG_xter_CRB.dat"%mut
...
ax1.legend() 
fig1.savefig('LowE_Nearby_Wat_pKa.pdf', format='pdf') 
...
fig2.savefig('LowE_Nearby_Wat_pKa_cryst.pdf', format='pdf') 
