...
if not os.path.isdir('cryst_2_pKas') : 
    print "Directory cryst_2_pKas does not exist; Run Cryst_2_pKa.sh to generate data"
    exit() 
else : 
...
ax1.plot(x, x*slope + intercept, '-',label="r^2 = %f"%r_value**2) 
...
#ax1.legend(handles=[circ, squar, diam],numpoints=1,loc=2) 
ax1.legend(handles=[best_fit],loc=2) 


fig1.savefig('cryst_pKas.pdf', fomat='pdf') 
