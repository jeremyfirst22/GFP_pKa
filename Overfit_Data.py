...
fileList = glob.glob('A_State/*/NEARBY_WAT_ENERGIES/dG_xter_CRO.dat') 
mutList = [] 
...
for index,mut in enumerate(mutList): 
    mut=mutList[index]
    outName = "Nearby_Wat_%s_ddG.pdf"%mut
    ...
    #print bashCommand
    dataFileA="../A_State/%s/NEARBY_WAT_ENERGIES/dG_xter_CRO.dat"%mut
    dataFileB="../B_State/%s/NEARBY_WAT_ENERGIES/dG_xter_CRB.dat"%mut
...
ax1.plot(x,x*slope+intercept, 'b-', label="r^2 = %.3f"%r_value**2) 
print "r = ", r_value, "\tr^2 = ", r_value**2, "\tslope = ",lope
...
ax1.legend() 
fig1.savefig('Nearby_Wat_pKa.pdf', format='pdf') 
...
fig2.savefig('Nearby_Wat_pKa_cryst.pdf', format='pdf') 
...
fig3.savefig('Nearby_Wat_residuals.pdf',format='pdf') 
