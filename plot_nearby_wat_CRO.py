...
datafiles = glob.glob('?_State/wat_near_CR?/*wat_near_CR?.xvg') 
...
for index in range(len(datafiles)/2)  : 
    name = datafiles[index].split('/')[2][:-17]
    names.append(name) 
    try : 
        dataA = np.genfromtxt(datafiles[index], skip_header=22) 
        dataB = np.genfromtxt(datafiles[index+len(datafiles)/2], skip_header=22) 
    except : 
        continue
