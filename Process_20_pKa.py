...
fileList = glob.glob('A_State/*/20_FREE_ENERGIES/dG_xter_CRO.dat') 
mutList = [] 
...
for index,mut in enumerate(mutList): 
    mut=mutList[index]
    outName = "20%s_ddG.pdf"%mut
    outData = "20%s_ddG_pKa.dat"%mut
    ...
    #print bashCommand
    dataFileA="../A_State/%s/20_FREE_ENERGIES/dG_xter_CRO.dat"%mut
    dataFileB="../B_State/%s/20_FREE_ENERGIES/dG_xter_CRB.dat"%mut
...
ax1.legend(loc=2) 
fig1.savefig('20pKa.pdf', format='pdf') 
...
ax2.set_ylabel(r"Calculated $pK_a$") 
ax2.set_xlabel(r"Experimental $pK_a$") 
...
fig2.savefig('20pKa_cryst.pdf', format='pdf') 
