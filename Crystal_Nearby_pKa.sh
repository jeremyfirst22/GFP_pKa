...
if [ ! -d cryst_nearby_pKas ] ; then mkdir cryst_nearby_pKas ; fi 
cd cryst_nearby_pKas
...
for molec in $molecList ; do 
    ...
    cp ../A_State/$molec/Solvate/temp.genion.tpr $molec.tpr 
    cp ../A_State/free_energy_files/AMBER* . 
    cp ../A_State/free_energy_files/20template.in . 
    cp ../A_State/GMXFF/*.dat . 
    cp -r ../A_State/GMXFF/amber03.ff . 

    printf "A State..."

    echo "group Protein || (group Water and same residue as within 0.5 of (resname CROn and name OH))" > selection.dat 
    cat selection.dat | gmx select -s $molec.tpr -f $molec.gro -ofpdb $molec.pdb -pdbatoms selected >> $logFile 2>> $errFile 
    check $molec.pdb 
