...
if [ ! -d no_wat_6_cryst_2_pKas ] ; then mkdir no_wat_6_cryst_2_pKas ; fi 
cd no_wat_6_cryst_2_pKas
...
for molec in $molecList ; do 
    printf "$molec:\t" 
    if [ ! -f ../A_State/$molec/Protein_min/$molec.gro ] ; then 
        echo "Skipped!"
        continue 
        fi
    cp ../A_State/$molec/Protein_min/$molec.gro  . 
    cp ../A_State/free_energy_files/AMBER* . 
    cp ../A_State/free_energy_files/6template.in . 
    ...
    pdb2pqr --userff=AMBER.DAT --usernames=AMBER.names --assign-only $molec+CRO.pdb $molec+CRO.pqr --drop-water >> $logFile 2>> $errFile 
    check $molec+CRO.pqr 
    ...
    sed "s/file2.pqr/$molec+CRO.pqr/" 6template.in > temp.in 
    ...
    pdb2pqr --userff=AMBER.DAT --usernames=AMBER.names --assign-only $molec+CRB.pdb $molec+CRB.pqr --drop-water >> $logFile 2>> $errFile
    check $molec+CRB.pqr 
    ...
    sed "s/file2.pqr/$molec+CRB.pqr/" 6template.in > temp.in 
